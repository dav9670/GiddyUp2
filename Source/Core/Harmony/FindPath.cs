...
namespace GiddyUp.Harmony
{
    ...
    class Patch_GetAllowedArea
    {
        static void Postfix(ref Area __result, Pawn pawn)
        {
			...
			var map = pawn.Map;
			ExtendedDataStorage.GUComp.areaNoMount.TryGetValue(map.uniqueID, out Area areaNoMount);
			...
			Area mergedArea = new Area_GU() { areaManager = map.areaManager, innerGrid = new BoolGrid() { trueCountInt = areaNoMount.innerGrid.trueCountInt, arr = areaNoMount.innerGrid.arr.ToArray() } };
			...
			if (__result != null)
			{
				...
				{
					if (!baseArr[i]) mergedArea.innerGrid.arr[i] = false;
				}
			}
